...
/**
 * Contains \Drupal\guestbook\Form\gbForm.
 *
 * @file
 */
 ...
 */
class GbChangeForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $times = time() + 3 * 60 * 60;
    $result = $connection->update('guestbook')
  ...
  public function setMessage(array $form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValue('vidguk') != NULL && $form_state->getValue('image')[0] > 0 && $is_nomer > 0 && $is_email > 0 && $is_name > 0) {
      $response->addCommand(new RedirectCommand('/admin/structure/gb'));
    }
    else {
      $response->addCommand(
        new HtmlCommand(
